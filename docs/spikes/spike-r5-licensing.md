...
### AC5 — 文件與已核准設計之一致性(round 2 新增)

> **本節為新增,非原始 declaration 的一部分。** 起因是第 2 輪 Layer 2 blocking(B-A):兩份文件把本專案**已核准的 RFC 決策**描述反了,其中一句還以「對 pipeline 具拘束力」之姿禁止了已核准的 D9。原考卷 16 項無法攔截這類缺陷——它只檢查「授權引文是否正確」,不檢查「文件對本專案自身設計的敘述是否正確」。
>
