...
3. **切點精確性**:切點在**被跨越的那一軸**取邊界的精確值,另一軸才線性內插。
   **理由不是**「讓相鄰兩側彼此位元相等」—— 那件事是免費的,與 snap 無關
   (Layer 2 round-2 S5,已量測,原本寫在這裡的理由為假)。兩側對同一道接縫的分子與分母
   恰好互為相反數(一側是 `high - start` / `+delta`,另一側是 `start - low` / `-delta`),
   IEEE 754 對 `(-a)/(-b)` 與 `a/b` 給出位元相同的商,所以兩側的 `t`、以及由 `t` 導出的
   每一個座標本來就一致 —— 四個量級帶共 80 萬次穿越,有無 snap 皆 0 次不同。
   snap 真正買到的是兩件事:(a) **與邊界常數本身相等** —— 那才是 `contains` 拿來比較、
   後續 tile 階段拿來量化與索引的值;(b) **與自己的歸屬判定一致** —— 純內插的切點會落到
   它剛被判定所屬的那條邊的另一側。M1 尺度、寬跨距各 20 萬次穿越:17,627 次未落在常數上,
   其中 8,675 次落在 `e_min` **之下**(`contains` 不認),8,952 次落在 `e_max` **之上**(整個出界)。
