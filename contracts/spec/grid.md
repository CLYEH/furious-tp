...
理由(實測值凍結於 `constants/m1_area.json` 的 `frame_survey`):本 bbox 的子午線收斂角(格網北與真北的夾角)為 0.2325–0.2475 度,在 500 m 邊長上即產生公尺級的旋轉差。以 per-tile 剛體 ENU 變換擺放時,tile 角落與逐頂點鏈相差 3.029 m;相鄰兩 tile 各自這樣擺放同一條共用邊上的點,實測裂縫 2.068 m。兩種讀法若都合法,接縫規則就形同虛設,故此處擇一寫死。
